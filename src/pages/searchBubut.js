...
const SearchCorrPage = ({ setTitle, setLoading }) => {
  ...
  const getBubut = async (value, die) => {
    setLoading(true);
    const dt = await odoo.getBubut(value, die);
    const result = [];
    dt.forEach((element) => {
      result.push(element);
    });
  ...
  const getInProgress = async (value, die) => {
    setLoading(true);
    const dt = await odoo.getCDIp('bubut', value, die);
    const result = [];
    dt.forEach((element) => {
      result.push(element);
    });
