...
const InternalTransferPage = ({ setTitle, setLoading, setMsgBox }) => {
  ...
  const comboBox = (tyype, label, change) => {
    ...
    React.useEffect(() => {
      ...
      (async () => {
        let dt = [];
        if (tyype === 'machine') {
          dt = await odoo.getMachine(false);
        } else {
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            ...
            label={label}
            // onChange={async (e) => {
            //   if (tyype === 'machine' && e.target.value) {
  ...
  const getPickingType = async () => {
    const type = await odoo.getPickingType('%Transfer%');
    try {
      if (type) {
        const data = [];
        type.forEach((item) => {
      ...
      <div className={classes.root}>
        <>
          ...
          <Grid container spacing={1}>
            ...
            <Grid>
              <Grid item xs={6} md={6} sm={12}>
                {comboBox('machine', 'Machine', (event, value) => {
                  if (value) {
                    setPicking({
                      ...picking,
                      machine_id: value.id,
              ...
              <Grid item xs={6} md={6} sm={12}>
                {comboBox('shift', 'Shift', (event, value) => {
                  if (value) {
                    setPicking({
                      ...picking,
                      shift_id: value.id,
...
InternalTransferPage.propTypes = {
